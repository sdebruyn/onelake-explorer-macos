...
/// Canonical on-disk locations the daemon exposes via "status".
/// Paths live inside ~/Library/Group Containers/group.dev.debruyn.ofem/,
/// which the host app can reach through its App Group entitlement.
...
public struct StatusInfo: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        paths = (try c.decodeIfPresent(StatusPaths.self, forKey: .paths)) ?? StatusPaths.empty
    }
}

extension StatusPaths {
    static let empty = StatusPaths(configFile: "", cacheDir: "", logDir: "")

    private init(configFile: String, cacheDir: String, logDir: String) {
...
public struct ConfigInfo: Decodable {
    ...
    private enum CodingKeys: String, CodingKey {
        case telemetry, defaultAccount, cache
    }

    private struct CacheConfig: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        defaultAccount = (try c.decodeIfPresent(String.self, forKey: .defaultAccount)) ?? ""
        let cache = try c.decodeIfPresent(CacheConfig.self, forKey: .cache)
        cacheMaxBytes = cache?.maxSizeBytes ?? 0
    }
}
