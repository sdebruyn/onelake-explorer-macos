...
- The **daemon** runs as a LaunchAgent (`OneLake.app/Contents/Library/LaunchAgents/dev.debruyn.ofem.daemon.app/Contents/MacOS/ofem`, registered via SMAppService) and is the single owner of the Go engine, cache, and blob store. It holds the Unix socket the host app and the File Provider Extension share, batches telemetry, and runs adaptive polling against Fabric.
