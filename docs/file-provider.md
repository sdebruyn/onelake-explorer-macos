...
- **Enumeration is single-page — no `NSFileProviderPage` cursor.** `OfemFPEEnumerator.enumerateItems` always reports one page with a `nil` cursor. This is correct-by-design, not an unfinished feature: by the time it runs, the folder is already fully materialized in memory. A cache hit is one unbounded `CacheReader.children(of:)` `fetchAll` (no `LIMIT`); a cold cache runs a blocking `refreshFolder`, whose `fetchRemoteChildren` step drives `OneLakeClient.listPath`'s DFS continuation-token loop to completion (looping until OneLake stops returning `x-ms-continuation`, up to `maxPaginationPages`) before `SyncEngine.enumerate(key:)` returns at all. Paging at the FPE layer would only slice up an array that's already fully built — it would save nothing on the network round trip or the peak memory of the refresh that already ran. It would also add a real new risk: a positional cursor over a cache that keeps mutating underneath it (concurrent `refreshFolder`/`refreshMaterialized` writes) is structurally the same cursor-stability hazard class the sync-anchor delta logic exists to solve for changes — except for reads, and without that logic's anchor/tombstone-horizon machinery behind it. Deferred until a real folder-size or memory problem provides evidence it's worth building that machinery for the read path too.

## Design decisions: per-container delta is alias-wide, not container-scoped

`OfemFPEEnumerator.enumerateChanges` is invoked once per *container* (workspace, item, or path), but the delta it serves — via the shared `serveCacheDelta` helper — is not scoped to that container. `CacheReader.itemsChangedAfter(accountAlias:ns:)` filters only by `accountAlias`, so every container that asks receives the same alias-wide set of changed rows and tombstones, not just the ones that live under it.

This is intentional and safe:

- **An out-of-scope `didUpdate` is inert, not wrong.** The framework re-resolves each delivered item's true location via the item's own `parentItemIdentifier` rather than trusting the container the delta arrived through, so an item that actually lives elsewhere in the alias is simply attributed correctly — it doesn't get misfiled into the container that happened to receive it.
- **Scoping was rejected as a bad trade.** `deletion_tombstones` (see `CacheSchema`) has only `account_alias`, `identifier_string`, and `deleted_at_ns` — no `workspace_id`/`item_id` columns to filter deletions on. Scoping would mean either a schema migration adding those columns, or parsing `identifier_string` back into its parts at read time — a fragile round-trip through a string that was never meant to be decomposed again. Either change would touch the anchor/tombstone-horizon code, the most safety-critical logic in this file (`syncAnchorDecision`, the lagging-client guard), for a purely local win.
- **The redundancy is local and indexed, not a network cost.** `itemsChangedAfter` is a SQLite read against `idx_pm_synced_at`/`idx_dt_deleted_at`, not a remote call. Over-delivering rows to an unrelated container costs a few extra in-memory `OfemFPEItem`s that the framework discards on re-resolution — not a fetch.

Given the payoff is trimming an already-cheap local over-delivery, and the code it would touch is the most safety-critical path in the enumerator, this was left as-is.
...
**Working-set container: the empty baseline is correct-by-design.** `OfemWorkingSetEnumerator.enumerateItems` always returns `[]`. The working set's items arrive exclusively via `enumerateChanges`'s real cache-delta path (`serveCacheDelta`), never via a baseline listing. For the common case this costs nothing: a fresh/zero sync anchor makes `syncAnchorDecision` always `.serve`, so `itemsChangedAfter(ns: 0)` on the very next `enumerateChanges` call collapses into "everything" — the empty `enumerateItems` baseline is simply never the thing that populates the working set.

There is one narrow, self-healing gap. `syncAnchorDecision` expires (forcing a fresh call to the empty `enumerateItems`) any anchor older than `tombstonesPurgedThroughNs`, the ~30-day tombstone-purge horizon (`CacheStore.tombstoneTTLNs`, see "Deletion delivery model" below). A working-set mount that goes 30+ days without a change-observation poll landing inside that window has its anchor age past the horizon; on the next poll macOS re-baselines through the empty page, and any previously materialized item that isn't re-delivered by a subsequent `enumerateChanges` delta drops out of the working set — i.e. out of Spotlight indexing and badge-status coverage — until it is next touched. This affects Spotlight/badge coverage only: the files themselves stay fully browsable via ordinary folder enumeration, a separate and unaffected path, and coverage self-heals the moment the item is touched again (opened, modified, or picked up by a materialized-folder refresh) and produces a fresh delta.
