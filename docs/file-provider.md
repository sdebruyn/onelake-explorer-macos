...
- **Item resolution is cache-first.** `OfemKit.ItemResolution.resolveItem`/`.createItem` (used by the FPE's `item(for:)` and `createItem`/`modifyItem` entry points) try a cached row via the private `cacheFirstRecord` helper before going to the network, falling back to `SyncEngine.resolveItemType(for:)` only when the cache has nothing usable.
