...
// within this process (M-1 fix). Two separate OFEM processes (e.g. CLI
// + daemon) racing on first run can each generate a UUID; one will win
// the atomic rename and the other's events will carry a short-lived ID.
// That race is acceptable per docs/telemetry.md.
